...
function processNonCommand(message) {
    ...
    const [category, subcategory, description] = utils.parseExpenseInput(message.text) || []
    if (!category) return bot.sendMessage(new bot.classes.Message(message.chat.id, {
        text: 'Sorry, I didn\'t understand you. Please try again.'
    }))
    commands.new(message, [category, subcategory, description])
}
